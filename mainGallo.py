...
with open('datosGallo.csv', 'w') as archivo_csv:
  escritor_csv = csv.writer(archivo_csv, delimiter=';')

  escritor_csv.writerow(['Nombre', 'Precio', 'Fechas', 'Reseñas'])
  pass
with open('datosGeneros.csv', 'w') as archivo_csv:
  escritor_csv = csv.writer(archivo_csv, delimiter=';')

  escritor_csv.writerow(['Genero', 'Precio', 'Fechas', 'Reseñas'])
  pass
for i in generos:
  ...
  with open(archivo, 'a', newline='', encoding='utf-8') as archivo_csv:
    escritor_csv = csv.writer(archivo_csv, delimiter=';')
    if (i == url):
      for title, precio, reseña, fecha in zip(titulos, precios, reseñas,
                                              fechas):
      ...
      for title, precio, reseña, fecha in zip(titulos[:10], precios[:10],
                                              reseñas[:10], fechas[:10]):
